...
// marco de coordenadas ({\bf cartesiano}) de la vista
...
class ViewFrustum {
 public:
  ...
  float left, right,  // extension en X ({\ttbf left} < {\ttbf right})
      ...
      near,
      far;  // extension en Z (rama negativa) ($0$ < {\ttbf near} < {\ttbf far})
  Matriz4f
      matrizProy;  // matriz de proyección $P$: cc. cámara ==> cc. recortado
...
class Camara {
 public:
  ...
  void
  activar();  // fijar matrices {\ttbf MODELVIEW} y {\ttbf PROJECTION} de OpenGL
};
