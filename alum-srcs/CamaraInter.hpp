...
class CamaraInteractiva : public Camara {
 public:
  bool examinar;  // modo: {\bf true} --> examinar, {\bf false} --> primera
                  // persona (inicialmente {\bf false})
  bool
      perspectiva;  // true --> camara perspectiva, false --> cámara ortográfica
  float ratio_yx_vp;  // aspect ratio del viewport (alto/ancho)
  float longi,   // en modo examinar: ángulo (en radianes) en torno al eje Y
                 // (longitud) (inic. 0)
      lati;      // en modo examinar: ángulo (en radianes) en torno al eje X
                 // (latitud) (inic. 0))
  Tupla3f aten;  // en modo examinar: punto de atención (inicialmente el
                 // origen+dist)
  float dist,    // en modo examinar: distancia entre el punto de atención y el
                 // observador
      hfov_grad,  // para cámaras perspectiva: angulo de apertura horizontal
      dx,
      dy;  // desplazamientos 'subpixel' para antialiasing (0 por inicialmente)
  ...
  //    * longi_ini_grad, lati_ini_grad: longitud y latitud iniciales (en
  //    grados)
  ...
  CamaraInteractiva(bool examinar_ini, float ratio_yx_vp_ini,
                    ...
                    float hfov_grad_ini = 80.0, float dist_ini = 1.8);

  //  métodos para manipular (desplazar o rotar) la cámara
  void moverHV(float nh,
               float nv);  // desplazar o rotar la cámara en horizontal/vertical
  void desplaZ(float nz);  // desplazar en el eje Z de la cámara (hacia adelante
                           // o hacia detrás)
  ...
  // calcula el marco de referencia de la camara y la matriz de vista (mcv),
  // usando:
  ...
  void modoExaminar(
      const Tupla3f& pAten);  // fija punt. aten. y activa modo examinar
